...
The streaming API is sometimes more convenient than the in-memory API. It can also be more efficient since it uses less memory. Also, you can start processing output before Vcdiff has finished. Sometimes you have to use the streaming API in order to handle files that are too large to fit into your virtual address space (though note that some backends have size limitations apart from this).
...
Note that in some backends the source file handle must be backed by an C<lseek(2)>able file descriptor (in other words, a real file, not a pipe or socket). Vcdiff will throw an exception otherwise.
...
Here is an example using L<Sys::Mmap>:
    ...
    mmap(my $source_str, 0, PROT_READ, MAP_SHARED, $source_fh) || die $!;
    mmap(my $target_str, 0, PROT_READ, MAP_SHARED, $target_fh) || die $!;
