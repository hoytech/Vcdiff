...
Unless you are relying on features supported only in a specific backend, it's recommended that code that uses L<Vcdiff> write backend-agnostic code, ie:

    use Vcdiff;
    print Vcdiff::diff("hello", "hello world");

That way the selection of which backend to use is fully flexible.

If you're writing a module that depends on L<Vcdiff>, you can pick a backend when creating your module's dependency list. This way a (sophisticated) user can force a different backend at install-time if the one you chose doesn't work.

Even more importantly, writing backend-agnostic code allows users of your module to choose which backend to use by setting C<$Vcdiff::backend> (or localising as described above). Backend-agnostic code also permits the flexibility of using one backend for diffing and another for patching (again by localising C<$Vcdiff::backend> for specific operations).
...
Note that the L<Vcdiff::OpenVcdiff> module does this under the hood if source is a file handle.
...
Every time a test-case is verified, the source will be diffed with the target, the source will then be patched by the resulting delta, and the output compared to the source.
