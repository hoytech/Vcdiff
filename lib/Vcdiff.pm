...
L<Vcdiff> aims to be "the DBI" of VCDIFF implementations.
...
That way the selection of which backend to use is as flexible as possible.
...
Here is an example using L<Sys::Mmap>:
    ...
    munmap($source_str); ## resource leak if diff() throws exception
    munmap($target_str); ## ditto
