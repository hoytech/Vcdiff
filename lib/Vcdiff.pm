...
Every time a test-case is verified, the source will be diffed with the target, the source will then be patched by the resulting delta, and the output compared to the original source.

The tests currently verify a few basic cases up to a megabyte or so in length. I'd like to go through the respective backends' test-suites and copy any interesting corner cases so they can be re-applied to all other backends.
...
The point of this test is to verify that the deltas produced by each backend are compatible will all other backends.

For each combination of backend, the L<streaming()> tests above are applied
